...
from data_loaders.sequence_padding import PadSequenceCDRDataset, PadSequenceCDRSentenceDataset, PadSequenceNERCDRDataset, PadSequenceCDRFulltextJoinLabelDataset
        ...
        while ids < len(text_tokenized):
            if text_tokenized[ids] != e_start_ids:
                ...
                if use_entity_token:
                    ...
                    list_entity_token_type.append(self.entities_list[entities_pos_sorted[entity_check]['code']]['entity_type'])
                ids += 1
                while text_tokenized[ids] != e_end_ids:
                    masked_entities.append(entities_pos_sorted[entity_check]['code'])
                    ids += 1
...
def make_train_pretrain_ner_dataset(train_path, dev_path, use_entity_token=False, batch_size=16, shuffle=True, num_workers=0):
    import copy
    data = []
    tokenizer = get_tokenizer()
    with open(train_path, 'r') as f:
...
def make_train_joinlabel_dataset(train_path, dev_path, use_entity_token=False, batch_size=16, shuffle=True, num_workers=0):
    import copy
    data = []
    tokenizer = get_tokenizer()
    with open(train_path, 'r') as f:
