...
class PadSequenceALPSDataset():
    ...
    def __call__(self, batch):
        ...
        chemical_code_seqs = torch.stack(list(map(lambda x: x[2], batch)))
        disease_code_seqs = torch.stack(list(map(lambda x: x[3], batch)))
        other_code_seqs = torch.stack(list(map(lambda x: x[4], batch)))
        label = torch.stack(list(map(lambda x: x[5], batch)))
        return seqs, masked_entities_encoded_seqs, chemical_code_seqs, disease_code_seqs, other_code_seqs, label